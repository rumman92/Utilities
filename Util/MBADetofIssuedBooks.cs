...
namespace Utility
{
    public partial class MBADetofIssuedBooks : Form
    {
        ...
        private void MBADetofIssuedBooks_Load(object sender, EventArgs e)
        {
            try
            {
                string det = string.Format("select [Card Number] from MBAIssueConfirm");
                da = new SqlDataAdapter(det, con);


                da.Fill(ds, "MBAIssueConfirm");
                foreach (DataRow dr in ds.Tables["MBAIssueConfirm"].Rows)
                {
        ...
        private void search()
        {
            ...
            else
            {
                try
                {
                    string query = string.Format("select * from MBAIssueConfirm where [Card Number]='{0}'", textBox5.Text);
                    ...
                    string det = string.Format("select [Book Name] from MBADetails where [ID Number]='{0}'", textBox3.Text);
                    da = new SqlDataAdapter(det, con);
        ...
        private void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string query = string.Format("select * from MBAIssueConfirm where [Card Number]='{0}'", listBox1.SelectedItem.ToString());
                da = new SqlDataAdapter(query, con);
