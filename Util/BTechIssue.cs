...
namespace Utility
{
    public partial class BTechIssue : Form
    {
        ...
        private void BTechIssue_Load(object sender, EventArgs e)
        {
            try
            {
                if (comboBox1.Text != string.Empty)
                {
                    string query = string.Format("select [Book Name] from BTechDetails where [Stream]='{0}'", comboBox1.Text);
        ...
        void sre_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            try
            {
                ...
                else if (e.Result.Text == "close" && ActiveForm.Name == "BTechIssue")
                {
                    this.Close();
                    sre.Dispose();
                }
        ...
        private void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string query = string.Format("select * from BTechDetails where [Book Name] ='{0}'", listBox1.SelectedItem.ToString());
                da = new SqlDataAdapter(query, con);
        ...
        private void confirm()
        {
            ...
            {
                BTechIssueConfirm btechissueconfirm = new BTechIssueConfirm(textBox3.Text, comboBox1.Text);
                btechissueconfirm.Show();
            }
            else
            {
        ...
        private void search()
        {
            try
            {
                if (radioButton1.Checked)
                {
                    ...
                    string query = string.Format("select * from BTechDetails where [Book Name]='{0}'", textBox1.Text);
                    da = new SqlDataAdapter(query, con);
                ...
                {
                    string query = string.Format("select * from BTechDetails where [Author]='{0}'", textBox2.Text);
                    da = new SqlDataAdapter(query, con);
                ...
                {
                    string query = string.Format("select * from BTechDetails where [ID Number]={0}", textBox3.Text);
                    da = new SqlDataAdapter(query, con);
