...
namespace Utility
{
    public partial class BArchIssue : Form
    {
        ...
        private void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string query = string.Format("select * from BArchDetails where Name='{0}'", listBox1.SelectedItem.ToString());
        ...
        private void BArch_Load(object sender, EventArgs e)
        {
            comboBox1.Text = "B.Arch";

            try
            {
                string query = string.Format("select * from BArchDetails");
                da = new SqlDataAdapter(query, con);
        ...
        void sre_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            try
            {
                ...
                else if (e.Result.Text == "close" && ActiveForm.Name == "BArchIssue")
                {
                    this.Close();
                    sre.Dispose();
                }
        ...
        private void search()
        {
            try
            {
                if (radioButton1.Checked)
                {
                    ...
                    string query = string.Format("select * from BArchDetails where Name='{0}'", textBox1.Text);
                    da = new SqlDataAdapter(query, con);
                ...
                {
                    string query = string.Format("select * from BArchDetails where Author='{0}'", textBox2.Text);
                    da = new SqlDataAdapter(query, con);
                ...
                {
                    string query = string.Format("select * from BArchDetails where [ID Number]={0}", textBox3.Text);
                    da = new SqlDataAdapter(query, con);
        ...
        private void confirm()
        {
            try
            {
                ...
                {
                    BarchIssueConfirm barchissueconfirm = new BarchIssueConfirm(textBox3.Text);
                    barchissueconfirm.Show();
                }
                else
