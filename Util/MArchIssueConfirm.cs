...
namespace Utility
{
    public partial class MArchIssueConfirm : Form
    {
        ...
        public MArchIssueConfirm()
        {
            InitializeComponent();
        }

        public MArchIssueConfirm(string id)
        {
            InitializeComponent();
            textBox3.Text = id;
        }
        ...
        private void release()
        {
            try
            {
                ...
                string insert = string.Format("insert into MArchIssueConfirm values('{0}','{1}','{2}','{3}')", textBox1.Text, textBox2.Text, textBox3.Text, DateTime.Now.ToString());

                da.InsertCommand = new SqlCommand(insert, con);
                da.InsertCommand.ExecuteNonQuery();

                string status = string.Format("select [Number of Copies] from MArchDetails where [ID Number]={0}", textBox3.Text);
                ...
                string insertcopies = string.Format("update MArchDetails set [Number of Copies]={0} where [ID Number]={1}", number, textBox3.Text);
        ...
        private void MArchIssueConfirm_Load(object sender, EventArgs e)
        {
