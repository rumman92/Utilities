...
namespace Utility
{
    public partial class MArchDetOfIssuedBooks : Form
    {
        ...
        private void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string query = string.Format("select * from MArchIssueConfirm where [Card Number]='{0}'", listBox1.SelectedItem.ToString());
                da = new SqlDataAdapter(query, con);

                da.Fill(ds, "MArchIssueConfirm");

                foreach (DataRow dr in ds.Tables["MArchIssueConfirm"].Rows)
                {
                ...
                string det = string.Format("select Name from MArchDetails where [ID Number]='{0}'", textBox3.Text);
        ...
        private void search()
        {
            ...
            else
            {
                try
                {
                    string query = string.Format("select * from MArchIssueConfirm where [Card Number]='{0}'", textBox5.Text);
                    da = new SqlDataAdapter(query, con);
        ...
        private void MArchDetOfIssuedBooks_Load(object sender, EventArgs e)
        {
            try
            {
                string det = string.Format("select [Card Number] from MArchIssueConfirm");
                da = new SqlDataAdapter(det, con);
